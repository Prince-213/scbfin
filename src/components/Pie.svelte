<script lang="ts">
    ...
    import data from '../data' // or pass data to component as prop
    ...
    {#each wedges as wedge, i}
      ...
      <g text-anchor='middle' transform='translate({arcLabel.centroid(wedge)})'>
        <text font-size={fontSize}>
          ...
          <tspan x = '0' dy='1.1em' font-size=14 >${percent ? `${(yVals[i] * 100).toFixed(2)}%` : yVals[i].toLocaleString('en-US')}</tspan>
        </text>
      </g>
    {/each}
  </svg>
