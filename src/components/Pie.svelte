<script lang="ts">
	...
	var data = [
		{ ages: 'Current', count: $CurrentUser.current },
		{ ages: 'Savings', count: $CurrentUser.savings }
	];

	var viewWidth = window.innerWidth;

	const width = viewWidth < 1000 ? 520 : 600; // the outer width of the chart, in pixels
	...
	{#each wedges as wedge, i}
		...
		<g text-anchor="middle" transform="translate({arcLabel.centroid(wedge)})">
			<text font-size={fontSize}>
				<tspan font-weight="bold" font-size="15">{xVals[i]}</tspan>
				<tspan x="0" dy="1.1em" font-size="14"
